    ...
    <div class="lesson-page site-container">
        ...
        <div class="lesson-workspace">
            <article class="lesson-reading">
                <header class="lesson-reading__header"><div class="lesson-reading__meta"><p class="eyebrow">Lesson</p></div><h1>{{ $lessonTitle }}</h1><p class="quiet-note">{{ $module['purpose'] }}</p><div class="lesson-progress" data-topic-progress data-progress-key="{{ $progressKey }}" data-progress-state="{{ $progressState ?? 'not_started' }}"><span class="lesson-progress__status" data-progress-status>{{ $progressState === 'completed' ? 'Topic selesai' : 'Belum dimulai' }}</span><button class="button button--quiet" type="button" data-progress-action>{{ $progressState === 'completed' ? 'Topic selesai' : 'Tandai selesai' }}</button>@auth<form class="lesson-bookmark-form" method="POST" action="{{ route('bookmarks.toggle') }}" data-bookmark-form>@csrf<input type="hidden" name="content_type" value="topic"><input type="hidden" name="content_key" value="{{ $progressKey }}"><button class="button button--quiet" type="submit" data-bookmark-action aria-pressed="{{ $bookmarked ? 'true' : 'false' }}">{{ $bookmarked ? 'Tersimpan' : 'Simpan lesson' }}</button></form>@endauth</div></header>
