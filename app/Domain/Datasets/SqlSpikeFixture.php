...
final class SqlSpikeFixture
{
    ...
    public function payload(): array
    {
        ...
        if (($manifest['schema_version'] ?? null) !== 1
            ...
            throw new RuntimeException('Invalid Gate A SQL fixture manifest or data dictionary.');
        }
    ...
    {
        if (! File::exists($path)) {
            throw new RuntimeException("Missing Gate A SQL fixture file [{$path}].");
        }

        try {
            $value = json_decode(File::get($path), true, 512, JSON_THROW_ON_ERROR);
        } catch (JsonException $exception) {
            throw new RuntimeException("Invalid Gate A SQL fixture JSON [{$path}].", previous: $exception);
        }

        if (! is_array($value)) {
            throw new RuntimeException("Gate A SQL fixture JSON must decode to an array [{$path}].");
        }
